...
public class PrivateLibrary extends Library implements IUUID2 {
    ...
    //
    // This not a system Library, so it doesn't use the Account Role Object for any checks.
    // Any user can access this Library.
    //
    // If a Private Library is created based on a BookId, then it is for ONLY 1 Book.
    // This is called an ORPHAN Private Library.
    ...
    public Result<Book> checkOutBookToUser(Book book, User user) {
        ...
        if (this.info.findAllKnownBookIds().size() != 1) return new Result.Failure<>(new Exception("Orphan Private Library can only check-out 1 Book to Users, bookId: " + book.id));
